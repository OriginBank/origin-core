...
contract WETHBondDepository is Ownable {
  ...
  address public immutable sOHM; // token sOHM
  address public turbine; // receives ohm from turbine bond
  address public community; // receives ohm from invite bond
  ...
  function deposit(
    ...
    ITreasury(treasury).mintRewards(address(this), payout.add(_inviteProfit));
  ...
  function redeem(
    ...
    uint percentVested = percentVestedFor(_recipient, _id, false); // (blocks since last interaction / vesting term remaining)
  ...
  function stakeOrSend(
    ...
    if (!_stake) {
      if (_invite) {
        ...
        IERC20(OHM).approve(address(turbine), _amount);
   ...
   * @param _price uint
   * @return _newbcv uint
   * mul(8) = mul(5).mul(1e9).div(1e6)
   */
  function getNewBCV(uint _price) public view returns (uint _newbcv) {
    _newbcv = _price.mul(1e8).div(uint(assetPrice()))
      .div(debtRatio()).div(1e9);
  }
  ...
  // function assetPrice() public pure returns (int) {
  // // return 3514312400000;
  //   return 200012400000;
  // }
  ...
  /**
   * @notice cllculate amount of address staked sOHM
  ...
  function getStakedAmount(
    ...
    uint sOHMBal = IsOHM(sOHM).balanceOf(_address);
    IStaking.Claim memory info = IStaking(staking).warmupInfo(_address);
    uint gonsBal = IsOHM(sOHM).balanceForGons(info.gons);
    _stakedAmount = sOHMBal.add(gonsBal);
  }
